...
$(function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 660,
			height : 380,
			href : '${ctx}/productfield/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#productfieldAddForm');
					f.submit();
				}
			} ]
		});
	...
	function edit(id){
		...
		parent.$.modalDialog({
			...
			width : 660,
			height : 380,
			href : '${ctx}/productfield/editPage?id=' + id,
			buttons : [ {
				text : '修改',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#productfieldEditForm');
					f.submit();
				}
			} ]
		});
...
<%-- 					<th>属性类型:</th>
					<td><input name="typename" class="easyui-combobox" data-options="
						required:false,
						url:'${ctx}/productfield/typeCombox',
	                    method:'get',
	                    valueField:'id',
	                    textField:'typename',
	                    panelHeight:'auto',
	                    editable:false" value="-1"></td> --%>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.resourceList, '/productfield/add')}">
