...
<title>乐购对账</title>
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/reconciliation/dataGrid',
			fit:true,
			striped : true,
			rownumbers : true,
			pagination : true,
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 500,	
			height : 450,
			href : '${ctx}/reconciliation/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#reconciliationAddForm');
					f.submit();
				}
			} ]
		});
		...
		<form id="searchForm" enctype="multipart/form-data">
			<table>
				<tr>
					...
					<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true" onclick="searchFun();">查询</a><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true" onclick="cleanFun();">清空</a><a href="${ctx}/reconciliation/exportaccountinfo" class="easyui-linkbutton" data-options="iconCls:'icon-redo',plain:true">导出</a></td>
