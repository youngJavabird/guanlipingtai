package light.mvc.service.reconciliation.impl;
...
public class ReconciliationServiceImpl implements ReconciliationServiceI {
	...
	public List<Scoredetail> dataGrid(Scoredetail scoredetail, PageFilter ph) {
		...
		String sql = "select * from view_tesco a ";
	...
	public Long count(Scoredetail scoredetail, PageFilter ph) {
		...
		String sql = "select count(*) from view_tesco a where 1=1";
		if (StringUtil.isNotEmpty(scoredetail.getCreatedatetimeStart()+"")&&StringUtil.isNotEmpty(scoredetail.getCreatedatetimeEnd()+"")){
			...
			sql += " and unix_timestamp(ddate)>=unix_timestamp('"+date+"') ";
			sql += " and unix_timestamp(ddate)<=unix_timestamp('"+date2+"') ";
//			sql += " (unix_timestamp(c.usedate)>=unix_timestamp('"+date+"' )";
//			sql += " and unix_timestamp(c.usedate)<=unix_timestamp('"+date2+"')) ";
		}
...
//			sql += " (unix_timestamp(c.usedate)>=unix_timestamp('"+date+"' ))";
		...
		if (StringUtil.isNotEmpty(scoredetail.getCreatedatetimeEnd()+"")&&StringUtil.isEmpty(scoredetail.getCreatedatetimeStart()+"")) {
			...
			sql += "and unix_timestamp(ddate)<=unix_timestamp('"+date2+"') ";
//			sql += "  or";
//			sql += " (unix_timestamp(c.usedate)<=unix_timestamp('"+date2+"' ))";
